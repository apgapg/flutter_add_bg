...
public class ImagePickerDelegate
    ...
    @VisibleForTesting
    static final int REQUEST_EXTERNAL_IMAGE_STORAGE_ONLY_PERMISSION = 2360;
    @VisibleForTesting
    ...
    public void saveImageToGallery(MethodCall methodCall, MethodChannel.Result result) throws IOException {
        ...
        if (!permissionManager.isPermissionGranted(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            permissionManager.askForPermission(
                    Manifest.permission.WRITE_EXTERNAL_STORAGE, REQUEST_EXTERNAL_IMAGE_STORAGE_ONLY_PERMISSION);
            return;
        }
        saveImage();
    ...
    public boolean onRequestPermissionsResult(
        ...
        switch (requestCode) {
            case REQUEST_EXTERNAL_IMAGE_STORAGE_PERMISSION:
                ...
                break;
            case REQUEST_EXTERNAL_IMAGE_STORAGE_ONLY_PERMISSION:
                if (permissionGranted) {
                    saveImage();
                }
                break;
